...
def destroy(vars_file):
    ...
    # TODO: have state file as input down in main()
    remove_from_state = [
        "module.astronomer",
        "google_sql_user.airflow",
        "module.astronomer_gcp.output.bastion_proxy_command",
        "module.astronomer_gcp.local.postgres_airflow_password"]
    for resource in remove_from_state:
        subprocess.run(f"terraform state rm {resource}", cwd=_dir, check=True, shell=True)
